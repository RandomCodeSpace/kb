...
func TestRemoteLifecycleEndToEnd(t *testing.T) {
	...
	if _, _, code = runCmd(t, "add", "Beta", "--check", "step", "--user", "Alice"); code != 0 {
	...
	if out, _, code = runCmd(t, "update", "2", "--prio", "1", "--user", "Alice"); code != 0 || out != "updated #2 Beta\n" {
		t.Fatalf("remote update: code=%d out=%q", code, out)
	}
	if _, errS, code = runCmd(t, "done", "2", "--user", "Alice"); code != 1 || !strings.Contains(errS, "checklist items are still open") {
		t.Fatalf("remote gated done: code=%d stderr=%q", code, errS)
	}
	if out, _, code = runCmd(t, "done", "2", "--force", "--user", "Alice"); code != 0 || out != "moved #2 -> done\n" {
		t.Fatalf("remote forced done: code=%d out=%q", code, out)
	}

	// comments work remotely, author is the acting identity.
	if out, _, code = runCmd(t, "comment", "add", "1", "remote note", "--user", "Alice"); code != 0 || out != "commented c1 on #1\n" {
		t.Fatalf("remote comment add: code=%d out=%q", code, out)
	}
	out, _, code = runCmd(t, "comment", "list", "1", "--user", "Alice")
	if code != 0 || !strings.Contains(out, "alice") || !strings.Contains(out, "remote note") {
		t.Fatalf("remote comment list:\n%s", out)
	}

	// links and the blocker gate, remotely.
	if out, _, code = runCmd(t, "link", "1", "blocks", "2", "--user", "Alice"); code != 0 || out != "linked: #1 blocks #2\n" {
		t.Fatalf("remote link: code=%d out=%q", code, out)
	}
	if _, errS, code = runCmd(t, "restore", "2", "--user", "Alice"); code != 0 {
		t.Fatalf("restore failed: %s", errS)
	}
	if _, errS, code = runCmd(t, "done", "2", "--user", "Alice"); code != 1 || !strings.Contains(errS, "open blocker") {
		t.Fatalf("remote blocker gate: code=%d stderr=%q", code, errS)
	}
	if _, _, code = runCmd(t, "unlink", "2", "1", "--user", "Alice"); code != 0 {
	...
	if out, _, code = runCmd(t, "comment", "rm", "c1", "--yes", "--user", "Alice"); code != 0 || out != "deleted c1\n" {
		t.Fatalf("remote comment rm: code=%d out=%q", code, out)
	}
	if out, _, code = runCmd(t, "rm", "2", "--yes", "--user", "Alice"); code != 0 || out != "deleted #2 Beta\n" {
		t.Fatalf("remote rm: code=%d out=%q", code, out)
	}

	// The retired i-N indexes point at their replacement.
	if _, errS, code = runCmd(t, "done", "i1", "--user", "Alice"); code != 1 || !strings.Contains(errS, "stable number") {
	...
	if _, errS, code = runCmd(t, "view", "9", "--user", "Alice"); code != 1 || !strings.Contains(errS, "task not found") {
