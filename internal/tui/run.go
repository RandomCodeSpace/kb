...
// version is the build identifier the launch screen's meta row prints (spec
// section 10.6.5); the TUI cannot reach package main's build info, so the
// caller passes it in and an empty string renders the meta row with its left
// slot only. Program options are accepted for deterministic tests; production
// passes none.
func Run(
	st *store.Store,
	databasePath, user, version string,
	options ...tea.ProgramOption,
) (err error) {
	return runWithSettings(st, databasePath, user, version, func(ctx context.Context, path string) (versionWatcher, error) {
...
func runWithSettings(
	...
	return runProgram(st, databasePath, user, version, openWatcher, func(ctx context.Context) *settingsModel {
