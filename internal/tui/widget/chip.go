...
// width(Text) for the flat form, hovered or not: spec section 10.5.1 spends an
// underline on the body run precisely because a pill has no tier left to raise
// and no cell to spare on a bigger cap.
...
// plain keeps its hash prefix. hovered underlines the body run per section
// 10.5.1 and costs no cell in either form.
func Label(styles *theme.Styles, tag string, on theme.Slot, flat, hovered bool) string {
	...
	if !scoped || key == "" || value == "" {
		return Chip(styles, ChipOpts{
			Text: styles.Glyph.MarkTag + tag, Fill: fill, On: on, Flat: flat, Hovered: hovered,
		})
	}
	if flat {
		return Chip(styles, ChipOpts{Text: value, Fill: fill, On: on, Flat: true, Hovered: hovered})
	}
	return Chip(styles, ChipOpts{Text: value, Key: key + ":", Fill: fill, On: on, Hovered: hovered})
}
