...
verbs, and the MCP tools are unchanged. The web API gains one route,
`PUT /api/comments/{id}`, for editing a comment in place.
...
- Cards carry a light wash of their priority hue: red for high, amber for
  medium, none for low. The Cancelled column starts hidden (Display options
  turns it back on), and the "Add task" row sits under Todo only; the + in
  every column header still adds a card there.
