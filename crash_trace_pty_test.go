...
func TestCrashTracePanicProcess(t *testing.T) {
	if mode := os.Getenv("KB_CRASH_TRACE_PROBE"); mode != "" {
		runCrashProbeChild(t, mode)
		return
	}
	for _, mode := range []string{"update", "command"} {
		t.Run(mode, func(t *testing.T) {
			data := t.TempDir()
			output := runCrashProbeProcess(t, mode, data)
			assertCrashProbeOutput(t, mode, output)
			assertCrashProbeReport(t, mode, data)
		})
	}
}
...
func assertCrashProbeOutput(t *testing.T, mode, output string) {
	t.Helper()
	...
	if strings.Contains(output, "capture incomplete") {
		t.Fatalf("incomplete capture:\n%s", output)
	}
}

func assertCrashProbeReport(t *testing.T, mode, data string) {
	t.Helper()
