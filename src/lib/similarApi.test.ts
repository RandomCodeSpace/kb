...
describe('similar items', () => {
  ...
  it('returns coerced advisory matches and carries request details', async () => {
    ...
    await expect(getSimilar(identity, 'login redirect', 'card-1', ctrl.signal)).resolves.toEqual([
      { title: 'Similar card', via: 'card' },
    ]);
    ...
    expect(firstCall[0]).toBe('/api/similar?q=login+redirect&exclude=card-1');
